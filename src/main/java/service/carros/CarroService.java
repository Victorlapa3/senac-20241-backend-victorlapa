...
import model.repository.montadora.CarroRepository;
import model.seletor.carros.CarroSeletor;

public class CarroService {

	private MontadoraService montadoraService = new MontadoraService();
	private CarroRepository carroRepo = new CarroRepository();

	public List<Carro> consultarComSeletor(CarroSeletor seletor) throws CarrosException {
		List<Carro> carros = carroRepo.consultarComSeletor(seletor);
		if (!seletor.temFiltro()) {
			throw new CarrosException("Pelo menos um filtro deve ser preenchido.");
		}
		if (seletor.temFiltro()) {
			carros = carroRepo.consultarComSeletor(seletor);
		}
		if (!seletor.anoEhValido()) {
			throw new CarrosException("Os filtros de ano devem ser informados em pares (inicial e final).");
		}

		if (!seletor.valorEhValido()) {
			throw new CarrosException("Os filtros de valor devem ser informados em pares (inicial e final).");
		}
